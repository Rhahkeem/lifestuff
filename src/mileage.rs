...
pub fn handle_mileage_operations(mileage_args: &Mileage, _: bool) -> Result<()> {
    ...
    let delta = ((projected_mileage - (mileage_args.mileage as f32)) / mileage_per_day)
        .ceil()
        .abs();
    println!("Current mileage is {}, projected mileage is {projected_mileage}. Current mileage is {} by {delta} days ", mileage_args.mileage, if under_allowance {"under".italic().green() }else{"over".bold().red() },  );
    Ok(())
}
