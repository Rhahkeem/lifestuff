...
fn parse_input_date_dmy(input: &str, verbose: bool) -> Result<(u32, u8, u8), Error> {
    ...
    let year = tokens[year_idx].parse::<u32>().context(format!(
    ...
    let month = tokens[month_idx].parse::<u8>().context(format!(
    ...
    let date = tokens[date_idx].parse::<u8>().context(format!(
    ...
    Ok((if year < 100 { year + 2000 } else { year }, month, date))
}
...
impl DateTimeKeeper {
    ...
    pub fn apply_month_delta(&self, months_delta: i32) -> Result<Self> {
        ...
        } else if (current_month as i32 - leftovers as i32) < 1 {
            whole_years -= 1;
